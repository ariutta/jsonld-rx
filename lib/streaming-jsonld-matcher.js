...
var JsonldMatcher = function(jsonldRx) {
  ...
  // TODO fix this.
  var _runOnceGlobal = jsonldRx._runOnceGlobal;

  var createJsonldNormalizerStream = function(doc) {
    var observable = jsonldRx.normalize(doc);
    var stream = highland();
    RxNode.writeToStream(observable, stream, 'utf8');
    return stream;
  };

  var internalContext = jsonldRx.defaultContext || {};
          ...
          .flatMap(function(jsonldNormalizedPair) {
            return _textNormalizePair(jsonldNormalizedPair)
              .flatMap(function(textNormalizedPair) {
                // The line above needs to be a flatMap, because if it's not,
                // the result will have stray properties from the normalization;
                // they will not be cleaned up.
                return [
                  pair,
  ...
  function getFormattedForComparison(
    ...
    var sourceCache = cache.get(name);

    console.log('name');
    console.log(name);

    if (sourceCache) {
      console.log('retrieving cached - jsonldRx');
      /*
    ...
    console.log('nothing cached - jsonldRx');

    var source = RxNode.fromReadableStream(
        init()
      )
      .toArray()
    ...
    sourceCache = new Rx.ReplaySubject(1);
    var initialStream = highland();
    cache.set(name, sourceCache);
    RxNode.writeToStream(source, initialStream);
    //*
